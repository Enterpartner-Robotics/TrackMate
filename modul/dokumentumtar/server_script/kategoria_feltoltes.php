...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($van_e_result){
        if($van_e_result['aktiv'] == 1){
            ...
            $query = "UPDATE `TM_dokumentumtar_kategoria` SET `aktiv`= 1 WHERE `kategoria_id` = ".$van_e_result['kategoria_id']."";
            $result = kerdes($query);
            if($result){
                echo json_encode([
                    'status' => 'success',
                    'message' => 'Kategória sikeresen aktiválva!'
                ]);
                exit;
            }
            else{
                echo json_encode([
                    'status' => 'error',
                    'message' => 'Hiba történt a kategória aktiválásakor!'
                ]);
                exit;
            }
        }
    }
    $query = "INSERT INTO `TM_dokumentumtar_kategoria` (`kategoria_id`, `kategoria_nev`, `aktiv`) VALUES (NULL, '$kategoria_nev', '1')";
    $result = kerdes($query);
    ...
    if ($result) {
        ...
        echo json_encode([
            'status' => 'error',
            'message' => 'Hiba történt a kategória hozzáadásakor.'
        ]);
    }
}
