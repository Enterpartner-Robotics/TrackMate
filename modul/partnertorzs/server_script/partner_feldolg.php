...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "INSERT INTO `TORZS_partner` 
              (`partner_id`,`cegnev`, `szekhely`, `telefonszam`, `cegadoszam`, `datum`, `felvetel_datum`, `aktiv`) 
              VALUES 
              (NULL,'$cegnev', '$szekhely', '$telefonszam', '$adoszam', '$datum', '$felvetel_datum', 1)";
    ...
    if ($result) {
        echo json_encode([
            'status' => 'success',
            'message' => 'Partner sikeresen hozzáadva!'
        ]);
    } else {
        echo json_encode([
            'status' => 'error',
            'message' => 'Hiba történt a partner hozzáadásakor.'
        ]);
    }
} else {
    echo json_encode([
